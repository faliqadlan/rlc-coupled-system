...
scene = canvas(
    ...
    width=600,
    height=300,
    center=vector(0.5, -1, 0),
    background=color.white,
)

Hook = box(pos=vector(d / 2, 0, 0), size=vector(d + 0.5, 0.5, 0.5), color=color.yellow)
...
rod1 = cylinder(
    pos=vector(0, 0, 0),
    axis=vector(l * np.sin(tht1), -l * np.cos(tht1), 0),
    radius=0.05,
)
rod2 = cylinder(
    pos=vector(d, 0, 0),
    axis=vector(l * np.sin(tht2), -l * np.cos(tht2), 0),
    radius=0.05,
)

Spring = cylinder(
    pos=vector(l * np.sin(tht1) / 2, -l * np.cos(tht1) / 2, 0),
    axis=vector(
        d + l * np.sin(tht2) / 2 - l * np.sin(tht1) / 2,
        -l * np.cos(tht2) / 2 + l * np.cos(tht1) / 2,
        0,
    ),
    radius=0.05,
    color=color.orange,
...
while t < T:
    ...
    omega_1 = (
        omega_1 + (-g * np.sin(tht1) / l + k * np.sin(2 * (tht2 - tht1)) / (8 * m)) * dt
    )
    omega_2 = (
        omega_2 + (-g * np.sin(tht2) / l - k * np.sin(2 * (tht2 - tht1)) / (8 * m)) * dt
    )
